 ...
 */
export function blankMatches(array: Match[], count: number, round: number) {
 ...
 * @return {null|Player[]}
 */
export function pairCompare(p1: Player, p2: Player): null | Player[] {
 ...
 */
export function elim(matches: Match[], players: Player[], thirdPlace: boolean, startRound = 1) {
	const mod = Math.log2(players.length);
	const rem = Math.round(2 ** mod) % 2 ** Math.floor(mod);
	const brack = bracket(Math.floor(mod));
	let r = startRound;
	// Create first round if not power of 2.
	if (rem !== 0) {
		blankMatches(matches, rem, r);
		r++;
	}
	let playerCount = Math.floor(mod) - 1;
	// Make all other rounds.
	do {
		blankMatches(matches, 2 ** playerCount, r);
		if (playerCount !== Math.floor(mod) - 1)
			matches
				.filter(m => m.round === r - 1)
				.forEach(
					m =>
						(m.winnerPath = matches
							.filter(p => p.round === r)
							.find(n => n.matchNumber === Math.ceil(m.matchNumber / 2))!)
				);
		r++ && playerCount--;
	} while (r < startRound + Math.ceil(mod));
	// Assign players.
	matches
		.filter(r => r.round === startRound + Number(rem !== 0))
		.forEach((m, i) => {
			m.playerOne = players[brack[2 * i] - 1];
			m.playerTwo = players[brack[2 * i + 1] - 1];
			m.active = true;
		});
	...
	if (rem !== 0) {
		...
		winnerRound1.forEach((m, i) => {
			...
			const n = matches
				.filter(p => p.round === startRound + 1)
				.find(
					m =>
						(m.playerOne !== null && m.playerOne.id === playerTwo.id) ||
						(m.playerTwo !== null && m.playerTwo.id === playerTwo.id)
				)!;
 ...
 * @param {Player[]} players The players for the tournament.
 */
export function doubleElim(matches: Match[], players: Player[], startRound = 1) {
	const mod = Math.log2(players.length);
	const rem = Math.round(2 ** mod) % 2 ** Math.floor(mod);
	const brack = bracket(Math.floor(mod));
	let r = startRound;
	...
	if (rem !== 0) {
		...
		r2.forEach(x => {
			for (let z = 0; z < 2; z++) {
				if (brack[i] > 2 ** Math.floor(mod) - rem) {
					const y = r1[j];
					y.active = true;
					x.active = false;
					y.playerOne = x.playerOne;
					x.playerOne = null;
					y.playerTwo = players[2 ** (Math.floor(mod) + 1) - brack[i]];
					y.winnerPath = x;
					j++;
				}
				i++;
	...
	const less = matches
		.filter(m => m.round === startRound + 1 && (m.playerOne === null || m.playerTwo === null))
		.map(m => Math.ceil(m.matchNumber / 2));
	const great = matches
		.filter(m => m.round === startRound + 1 && m.playerOne === null && m.playerTwo === null)
		.map(m => m.matchNumber);
	// Create final winner bracket round (to play winner of loser's bracket).
	blankMatches(matches, 1, r);
	...
	if (rem !== 0) {
		if (rem <= 2 ** Math.floor(mod) / 2) {
			...
			blankMatches(matches, rem - 2 ** (Math.floor(mod) - 1), r);
			r++;
			blankMatches(matches, 2 ** (Math.floor(mod) - 1), r);
			r++;
		}
	}
	...
	if (rem === 0) {
		...
		const first = loserFill(winnerRound1.length, fillCount);
		fillCount++;
		let d = 0;
		...
		const second = loserFill(winnerRound2.length, fillCount);
		fillCount++;
		let d = 0;
		let e = 0;
		let lessCopy = [...less];
	...
	for (let i = winnerRound; i < diff; i++) {
		...
		const fill = loserFill(w.length, fillCount);
		fillCount++;
		l.forEach((x, i) => {
	...
	if (rem !== 0) {
		if (rem <= 2 ** Math.floor(mod) / 2) {
			...
			const n = matches.filter(r => r.round === diff + 1);
			n.forEach(
				(m, i) => (m.winnerPath = matches.filter(r => r.round === m.round + 1).find(z => z.matchNumber === great[i])!)
			);
		}
	}
	// Route winners in loser's bracket.
	for (let i = diff + 1 + Number(rem !== 0); i < matches.reduce((x, y) => Math.max(x, y.round), 0); i++) {
...
export function swiss(
	...
	let loopCheck = 0;
	let success;
	const pairAttempt = (p: number, u: Player[]) => {
		let matchNumber = 1;
		newMatches = [];
		do {
			...
			if (u.filter(x => x.matchPoints < p).length === 0) {
				...
				receivedByes.forEach(player =>
					thesePlayers.splice(
						thesePlayers.findIndex(x => x.id === player.id),
						1
					)
				);
 ...
 * @return {Match[]}
 */
export function dutch(matches: Match[], players: Player[], round: number, maxPoints: number): Match[] {
	let hold: Player[] = [];
	let newMatches: Match[] = [];
	let collapse = false;
	let loopCheck = 0;
		...
		do {
			...
			while (s1.length + s2.length > 1) {
				...
				while (i < s2.length) {
					...
					if (
						s1[0].results.findIndex(o => o.opponent === s2[i].id) > -1 ||
						(s1[0].colorPref === 2 && s2[i].colorPref === 2) ||
						(s1[0].colorPref === -2 && s2[i].colorPref === -2)
					)
						i++;
					else {
						// Determine if they can be paired, and who is player 1 and who is player 2.
						const order = pairCompare(s1[0], s2[i]);
						if (order === null) i++;
						else {
							let newId: string;
							do {
...
function is2DArray<T>(x: T[] | T[][]): x is T[][] {
 ...
 * @return {Match[]}
 */
export function robin(players: Array<Player | null> | Array<Player | null>[], double: boolean): Match[] {
	let matches: Match[] = [];
	// If we're pairing by groups.
	if (is2DArray(players)) {
		// Loop once or twice, depending on double.
		for (let i = 0; i < 1 + Number(double); i++) {
			// Loop through groups.
			players.forEach((g, j) => {
				...
				for (let k = 0; k < g.length - 1; k++) {
					...
					if (j === 0) blankMatches(matches, players.length * (g.length / 2), r);
		...
		for (let i = 0; i < 1 + Number(double); i++) {
			...
			for (let j = 0; j < thesePlayers.length - 1; j++) {
				blankMatches(matches, thesePlayers.length / 2, r);
				const newPairings = matches.filter(p => p.round === r);
				// Round 1 pairing method.
				if (j === 0) {
					...
					newPairings.forEach((m, k) => {
						...
						if (k === 0) {
							if (
								oldPairings[0].playerTwo === thesePlayers[thesePlayers.length - 1] ||
								oldPairings[0].playerTwo === null
							) {
								m.playerOne = thesePlayers[thesePlayers.length - 1];
								let i1 = thesePlayers.findIndex(p => p!.id === oldPairings[0].playerOne!.id);
								i1 =
									i1 + thesePlayers.length / 2 > thesePlayers.length - 2
										? i1 - thesePlayers.length / 2 + 1
										: i1 + thesePlayers.length / 2;
							...
							i2 =
								i2 + thesePlayers.length / 2 > thesePlayers.length - 2
									? i2 - thesePlayers.length / 2 + 1
									: i2 + thesePlayers.length / 2;
