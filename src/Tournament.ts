...
export class Swiss extends Tournament {
	...
	startEvent() {
		...
		this.currentRound++;
		if (this.dutch)
			this.matches = this.matches.concat(Algorithms.dutch(this.matches, this.players, this.currentRound, 0));
		else {
	...
	result(match: Match, playerOneWins: number, playerTwoWins: number, draws: number = 0): Match[] | null {
		...
		if (this.currentRound > this.numberOfRounds!) {
			if (match.winnerPath !== null) {
				...
				else if (match.winnerPath.playerTwo === null)
					match.winnerPath.playerTwo = playerOneWins >= playerTwoWins ? match.playerOne : match.playerTwo;
			...
			if (match.loserPath !== null) {
				if (match.loserPath.playerOne === null)
					match.loserPath.playerOne = playerOneWins < playerTwoWins ? match.playerOne : match.playerTwo;
	...
	nextRound(): Match[] | boolean {
		...
		if (this.currentRound === this.numberOfRounds) {
			if (this.playoffs !== null) {
				...
				} else if (this.cutType === "points" && this.cutLimit > 0)
					this.players.filter(p => p.matchPoints < this.cutLimit).forEach(p => this.removePlayer(p));
				if (this.playoffs === "elim")
					Algorithms.elim(
						this.matches,
			...
			if (this.dutch)
				this.matches = this.matches.concat(
					Algorithms.dutch(
						...
						this.players.filter(p => p.active),
						this.currentRound,
						this.winValue * (this.currentRound - 1)
					)
				);
			else {
				const seedPref = this.seededPlayers ? this.seedOrder : null;
...
export class RoundRobin extends Tournament {
	...
	startEvent() {
		...
		byes.forEach(b =>
			b.playerOne === null
				? this.result(b, 0, Math.ceil(this.bestOf / 2))
				: this.result(b, Math.ceil(this.bestOf / 2), 0)
		);
		...
		if (this.currentRound === this.numberOfRounds) {
			if (this.playoffs !== null) {
				...
				if (this.cutType === "rank" && this.cutLimit > 0) {
					...
					this.players.filter(p => p.matchPoints < this.cutLimit).forEach(p => this.removePlayer(p));
				if (this.playoffs === "elim")
					Algorithms.elim(this.matches, this.standings(), this.thirdPlaceMatch, this.currentRound);
...
export class Elimination extends Tournament {
	...
	result(
		...
		if (match.loserPath !== null && dropdown) {
			...
			if (
				match.loserPath.playerOne !== null &&
				match.loserPath.playerTwo !== null &&
				match.loserPath.playerTwo !== undefined
			) {
				match.loserPath.active = true;
				newMatches.push(match.loserPath);
			}
